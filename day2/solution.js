...
//part 2 solution
//"memory" parameter will be the puzzle input
const getNounVerb = (memory = []) => {
    let output = 19690720
    let result = 0

    for (let noun = 0; noun < 100; noun++) {
        for (let verb = 0; verb < 100; verb++) {
            result = restoreGravityAssistProgram([...memory], [noun, verb])
            if (result === output) {
...
console.log(restoreGravityAssistProgram([...intcodeValues], [...overrideValues]), '<--- part 1 answer')
console.log(getNounVerb([...intcodeValues]), '<--- part 2 answer')